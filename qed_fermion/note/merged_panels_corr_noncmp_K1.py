...
def plot_slope_vs_invL_spsm(ax):
    ...
    with open(os.devnull, 'w') as devnull:
        with contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            ...
            try:
                mod = importlib.import_module('qed_fermion.observables.spin_r_noncmpK1_large1_spsm_sup_repr2.plot_slope_vs_invL')
...
def plot_slope_vs_invL_bond(ax):
    ...
    with open(os.devnull, 'w') as devnull:
        with contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            saved = _inject_ax_context_for_second_figure(ax)
            try:
                mod = importlib.import_module('qed_fermion.observables.spin_r_noncmpK1_large8_bond_pi_flux.plot_slope_vs_invL')
                if hasattr(mod, 'plot_slope_vs_invL'):
                    mod.plot_slope_vs_invL()
            finally:
                _restore_context(saved)
...
def plot_flux_slope_vs_invL(ax):
    ...
    with open(os.devnull, 'w') as devnull:
        with contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            saved = _inject_ax_context_for_second_figure(ax)
            try:
                mod = importlib.import_module('qed_fermion.observables.spin_r_noncmpK1_large1_spsm_sup_repr2.plot_flux_slope_vs_invL')
...
def main():
    ...
    from qed_fermion.observables.spin_r_noncmpK1_large1_spsm_sup_repr2.plot_flux_plot import plot as plot_flux
