...
def plot_energy_J(Lx, Ltau, Js=[], starts=[500], sample_steps=[1]):
    ...
    # save plot
    method_name = "boson"
    save_dir = os.path.join(script_path, f"./figures{Lx}_{Ltau}/energies_boson")
    ...
    # save plot
    method_name = "fermion"
    save_dir = os.path.join(script_path, f"./figures{Lx}_{Ltau}/energies_fermion")
    os.makedirs(save_dir, exist_ok=True) 
    file_path = os.path.join(save_dir, f"{method_name}.pdf")
    plt.savefig(file_path, format="pdf", bbox_inches="tight")
    print(f"Figure saved at: {file_path}")
...
if __name__ == '__main__':
    ...
    plot_energy_J(Lx, Ltau, Js=Js, starts=[start_hmc], sample_steps=[1])
