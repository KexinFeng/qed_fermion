...
def postprocess_and_write_spsm(bosons, output_dir, Lx, Ly, Ltau, Nrv=10, mxitr=200, start=5000):
    ...
    se.batch_size = lambda nrv: min(int(nrv * 400), 100*400)
    ...
    for boson in tqdm(boson_seq):  # boson: [J/bs, 2, Lx, Ly, Ltau]
        ...
        if se.cuda_graph_se:
            obsr = se.graph_runner(boson, eta, indices, indices_r2)
        else:
            obsr = se.get_fermion_obsr(boson, eta, indices, indices_r2)
