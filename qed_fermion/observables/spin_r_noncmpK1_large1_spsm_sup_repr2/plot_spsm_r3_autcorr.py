...
def plot_spsm_r_autocorr():
    """Plot autocorrelation-k curves for spsm_r at r=3 for different lattice sizes."""
    ...
    for i, Lx in enumerate(lattice_sizes):
        ...
        # Check if r=3 is within the lattice
        if R_VALUE >= Lx:
            print(f'Lx={Lx}: r={R_VALUE} exceeds lattice size, skipping')
            continue

        # Extract spsm_r at r=R_VALUE (at y=0, x=R_VALUE)
        # Average over batch dimension (axis=1) and take absolute value
        spsm_r_at_r = spsm_r[:, :, 0, R_VALUE].abs().mean(axis=1).cpu().numpy()
        
        # Determine equilibrium threshold based on mean of later timesteps
        if Lx >= 30:
            equilibrium_threshold = spsm_r_at_r[5000:6000].mean() * 1.07
        else:
            equilibrium_threshold = spsm_r_at_r[3000:4000].mean() * 1.07
        
        # Find equilibrium point: first time spsm_r_at_r <= threshold
        equilibrium_indices = np.where(spsm_r_at_r <= equilibrium_threshold)[0]
        if len(equilibrium_indices) == 0:
            print(f'Lx={Lx}: No equilibrium point found (spsm_r never reached {equilibrium_threshold})')
            continue
        
        equilibrium_start_idx = equilibrium_indices[0]
        equilibrium_data = spsm_r_at_r[equilibrium_start_idx:]
    ...
    # Save the plot
    save_dir = os.path.join(script_path, "./figures/spsm_r_autocorr")
    os.makedirs(save_dir, exist_ok=True)
    file_path = os.path.join(save_dir, "spsm_r3_autocorr_noncmpK1.pdf")
