...
a = torch.randn((1,) + shape, dtype=torch.cfloat, device=device)  # [1, Ltau, Ly, Lx]
b = torch.randn((1,) + shape, dtype=torch.cfloat, device=device)
...
G = torch.einsum('bi,bj->ji', a_flat.conj(), b_flat) / a_flat.shape[0]  # [N, N]
...
for i in range(N):
    for d in range(N):
        ...
        result[i, d] = G[idx, i]
G_mean_LHS = result.mean(dim=0).view(shape[::-1])  # [Lx, Ly, Ltau]
